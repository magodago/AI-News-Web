document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---------------------------
     13. Efecto Bola Neurona y Onda Expansiva
  --------------------------- */
  // Función para mover una neurona
  function moverNeurona(neuron) {
    if (!neuron.vx) {
      neuron.vx = 1 + Math.random() * 2;
      neuron.vy = 1 + Math.random() * 2;
    }
    function animate() {
      let x = parseFloat(neuron.style.left) || 0;
      let y = parseFloat(neuron.style.top) || 0;
      x += neuron.vx;
      y += neuron.vy;
      if (x <= 0 || x >= window.innerWidth - neuron.offsetWidth) { neuron.vx *= -1; }
      if (y <= 0 || y >= window.innerHeight - neuron.offsetHeight) { neuron.vy *= -1; }
  ...
  // Función para crear el efecto de onda expansiva
  ...
  // Inicializar la neurona (bola)
  const neuron = document.getElementById("glowing-ball");
  if (neuron) {
    // Posición inicial aleatoria
    neuron.style.left = (Math.random() * (window.innerWidth - 40)) + "px";
    neuron.style.top = (Math.random() * (window.innerHeight - 40)) + "px";
    moverNeurona(neuron);
    // Al hacer clic sobre la neurona
    neuron.addEventListener("click", (e) => {
      // Crear efecto de onda en la posición del click
      createWaveEffect(e.clientX, e.clientY);
      // Clonar la neurona para duplicarla
      const newNeuron = neuron.cloneNode(true);
      newNeuron.removeAttribute("id"); // Evitar duplicar el id
      newNeuron.style.left = (Math.random() * (window.innerWidth - 40)) + "px";
      newNeuron.style.top = (Math.random() * (window.innerHeight - 40)) + "px";
      document.body.appendChild(newNeuron);
      // Agregar el mismo efecto de clic al clon
      newNeuron.addEventListener("click", (e) => {
        createWaveEffect(e.clientX, e.clientY);
        // Puedes duplicar nuevamente si se desea
        const anotherNeuron = newNeuron.cloneNode(true);
        anotherNeuron.removeAttribute("id");
        anotherNeuron.style.left = (Math.random() * (window.innerWidth - 40)) + "px";
        anotherNeuron.style.top = (Math.random() * (window.innerHeight - 40)) + "px";
        document.body.appendChild(anotherNeuron);
        moverNeurona(anotherNeuron);
      });
      moverNeurona(newNeuron);
    });
  }

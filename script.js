document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ---------------------------
     13. (Opcional: Aquí se mantiene el simulador de futuro y demás secciones...)
     [El código de las secciones restantes se mantiene sin cambios]
  --------------------------- */
  
  /* ---------------------------
     14. NUEVA SECCIÓN: Bola Fugaz con Ojos y Boca
       Se crea un elemento #fugaz-ball que se mueve aleatoriamente por la página.
  --------------------------- */
  const fugazBall = document.getElementById("fugaz-ball");
  if (fugazBall) {
    // Posición inicial aleatoria
    let ballX = Math.random() * (window.innerWidth - 50);
    let ballY = Math.random() * (window.innerHeight - 50);
    fugazBall.style.left = ballX + "px";
    fugazBall.style.top = ballY + "px";
    // Velocidad (puedes ajustar para que sea "fugaz")
    let ballVX = 4 + Math.random() * 4;
    let ballVY = 4 + Math.random() * 4;
    function animateFugazBall() {
      ballX += ballVX;
      ballY += ballVY;
      if (ballX <= 0 || ballX >= window.innerWidth - 50) { ballVX *= -1; }
      if (ballY <= 0 || ballY >= window.innerHeight - 50) { ballVY *= -1; }
      fugazBall.style.left = ballX + "px";
      fugazBall.style.top = ballY + "px";
      requestAnimationFrame(animateFugazBall);
    }
    animateFugazBall();
  }
});
